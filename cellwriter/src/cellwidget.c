...
/**insert_button_clicked 선언**/
void insert_button_clicked(void);  
...
/* Cell preferences */
int cell_width = 500, cell_height = 500, cell_cols_pref = 1, cell_rows_pref = 1,
	enable_cairo = TRUE, training = FALSE, train_on_input = TRUE,
	right_to_left = FALSE, keyboard_enabled = 0, xinput_enabled = FALSE;
...
static void finish_cell(int cell)
{
		...
		/* Recognize input */
				...
				/*키보드 디바이스 드라이버 이벤트 연결*/				
				//device_event(pc->ch,EV_PRESSED,KEYBOARD);
				keyboard_event(pc->ch);
				if (pc->ch)
						pad_cell(cell);
...
/*
void select_key(int fd, unsigned int my_key){
		if(my_key == 0x3131)
				...
				event_key_start.code=KEY_LEFTSHIFT;
...
void keyboard_event(int fd, gunichar ch)
{
...
void mouse_event(int fd, int code,int value){
